...
function TrafficBars({ compact = false }: { compact?: boolean }) {
  ...
  return <div className={compact ? 'h-48' : 'h-64'}><div className="flex h-full items-end gap-3 border-b border-stroke-soft-200 px-1 pt-8">{dailyTraffic.map((item) => <div key={item.day} className="group flex h-full flex-1 flex-col items-center justify-end gap-2"><div className="relative w-full max-w-14 rounded-t-lg bg-blue-100 transition group-hover:bg-primary-base" style={{ height: `${(item.value / maximum) * 86}%` }}><span className="absolute -top-6 left-1/2 -translate-x-1/2 text-[10px] font-bold text-text-sub-600">{item.value.toFixed(1)}</span></div><span className="pb-2 text-[10px] text-text-soft-400">{item.day}</span></div>)}</div></div>;
}
  ...
  return <>
    ...
    <div className="grid gap-4 xl:grid-cols-[1.65fr_1fr]">
      ...
      <Card><CardHeader title="Distribuição por função" description="Participação no tráfego mensal" /><CardBody className="space-y-5">{[
        ['Vídeo sob demanda', 56, 'var(--color-primary-base)'], ['Eventos automáticos', 23, 'var(--color-feature-base)'], ['Transmissão ao vivo', 14, 'var(--color-warning-base)'], ['Dados de telemetria', 7, 'var(--color-success-base)'],
      ].map(([label, value, color]) => <div key={String(label)}><div className="mb-2 flex justify-between text-xs"><span className="font-medium text-text-sub-600">{label}</span><strong>{value}%</strong></div><div className="h-2 overflow-hidden rounded-full bg-bg-weak-50"><div className="h-full rounded-full" style={{ width: `${value}%`, backgroundColor: String(color) }} /></div></div>)}</CardBody></Card>
      <Card className="xl:col-span-2"><CardHeader title="Veículos com maior consumo" description="Ranking mensal por linha de dados" action={<Button variant="ghost" onClick={() => navigate('/ftv/flow/flow-manage')}>Abrir lista <RiArrowRightLine className="size-4" /></Button>} /><DataTable data={traffic.slice(0, 8)} rowKey="id" initialPageSize={5} columns={[
        { key: 'plate', header: 'Veículo', sortable: true, render: (row) => <strong className="text-text-strong-950">{row.plate}</strong> },
...
export function DeviceDataListPage() {
  ...
  return <>
    ...
    <Card><DataTable data={filtered} rowKey="id" selectable onRowClick={() => navigate('/ftv/flow/flow-manage/detail')} columns={[
      { key: 'plate', header: 'Veículo', sortable: true, render: (row) => <div><strong className="block text-text-strong-950">{row.plate}</strong><span className="text-[10px] text-text-soft-400">{row.device}</span></div> },
