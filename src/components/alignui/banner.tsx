...
import { PolymorphicComponentProps } from '@/components/alignui/utils/polymorphic';
import { recursiveCloneChildren } from '@/components/alignui/utils/recursive-clone-children';
import { tv, type VariantProps } from '@/components/alignui/utils/tv';
...
export const bannerVariants = tv({
  ...
  variants: {
    variant: {
      ...
      stroke: {
        root:
          'bg-bg-white-0 text-text-strong-950 before:absolute before:bottom-0 before:h-px before:w-full before:bg-stroke-soft-200',
      },
    },
    status: {
      error: {},
...
function Banner({
  ...
  const sharedProps: BannerSharedProps = { variant, status };
